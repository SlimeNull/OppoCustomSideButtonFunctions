...
摩斯电码模式与简单操作互斥。`0` 表示按下时间短于“长按持续时间”，`1` 表示达到该时间；默认阈值为 350ms。“尽可能立即执行操作”默认开启，当前序列精确匹配且不存在更长的同前缀指令时立即执行；关闭后所有序列都等待“指令判断等待时间”（默认 350ms）。错误序列在等待期间继续累积输入，窗口结束后整体丢弃，避免后续按键被误识别为另一条指令。可以为任意长度的 `0/1` 序列分别配置动作。按下和达到长按阈值时各有独立的 50ms 振动开关，前者默认关闭，后者默认开启。
